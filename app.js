...
containers.forEach((container) => {
    ...
    input.addEventListener("focus", () => {
        // run animation only if input empty
        if (!input.value) {
            tl.fromTo(
                line,
                {
...
// revert back placeholder if not focused and empty input
form.addEventListener("click", () => {
    containers.forEach((container) => {
        ...
        if (document.activeElement !== input) {
            // if current input is not focused and empty, revert back animation
            if (!input.value) {
                gsap.to(placeholder, {
                    top: 0,
                    left: 0,
        ...
        input.addEventListener("input", (e) => {
            ...
            if (e.target.type === "tel") {
                let isValidTel = validatePhone(e.target.value);
                if (isValidTel) {
                    colorize("#6391E8", line, placeholder);
                } else {
                    colorize("#FE8C99", line, placeholder);
